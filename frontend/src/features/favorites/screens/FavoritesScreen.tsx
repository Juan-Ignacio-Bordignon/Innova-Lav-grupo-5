...
export function FavoritesScreen() {
  ...
  try {
    ...
    const [favoritesResult, userResult] = await Promise.allSettled([
      getFavorites(),
      getCurrentUser(),
    ]);

    // El usuario se carga independientemente de Favoritos.
    if (userResult.status === 'fulfilled' && userResult.value) {
  const username = userResult.value.usuario?.username?.trim();
  ...
  setUserName('Usuario');
    ...
    // La lista de favoritos también se maneja de forma independiente.
  ...
  const handleFavoritePress = (favorite: FavoriteItem) => {
    navigation.navigate(ROUTES.EXERCISE, {
      ...
      exerciseTitle: favorite.title,
      contenidoMultimedia: favorite.videoUrl,
    });
  };

  const handleRemoveFavorite = async (favorite: FavoriteItem) => {
    if (removingIds.has(favorite.exerciseId)) {
      return;
    }
    ...
    setRemovingIds((current) => {
      ...
      next.add(favorite.exerciseId);
      return next;
    });

    try {
      await removeFavorite(favorite.exerciseId);
      setFavorites((current) =>
        current.filter(
          (item) => item.exerciseId !== favorite.exerciseId
        )
      );
    } catch (error) {
      setActionErrorMessage(
      ...
      setRemovingIds((current) => {
        const next = new Set(current);
        next.delete(favorite.exerciseId);
        return next;
      });
    }
  };
  ...
  const handleExploreModules = () => {
    ...
    if (tabsNavigation) {
      tabsNavigation.navigate(ROUTES.HOME);
      return;
    }
    ...
    <View style={styles.headerSection}>
      <AnimatedEntry delay={150} triggerKey={animationKey}>
        ...
        <AppText variant="body" style={styles.subtitle}>
          Guardá tus señas y volvé a practicarlas cuando quieras.
        </AppText>
      </AnimatedEntry>
      ...
      <AnimatedEntry delay={290} triggerKey={animationKey}>
        <AppInput
          value={searchText}
      ...
      {actionErrorMessage ? (
        <AnimatedEntry delay={330} triggerKey={animationKey}>
          <View style={styles.actionErrorCard}>
            <MaterialIcons
              ...
              color={colors.error}
            />
            <AppText style={styles.actionErrorText}>
              {actionErrorMessage}
            </AppText>
          </View>
        </AnimatedEntry>
      ...
      {!isLoading && !errorMessage && favorites.length > 0 ? (
        <AnimatedEntry delay={340} triggerKey={animationKey}>
          <View style={styles.listTitleRow}>
            <AppText style={styles.listTitle}>Para repasar</AppText>
  ...
  const renderFavorite = ({
    ...
    const isRemoving = removingIds.has(item.exerciseId);

    return (
      <AnimatedEntry
        delay={390 + index * 80}
        triggerKey={animationKey}
        style={styles.favoriteAnimationWrapper}
      >
        <Pressable
        ...
        >
          ...
          <View style={styles.favoriteContent}>
            ...
            >
              {item.description || 'Tocá para volver a practicar esta seña.'}
            </AppText>
          </View>
  ...
  const renderEmpty = () => {
    ...
    if (errorMessage) {
      return (
        <AnimatedEntry delay={390} triggerKey={animationKey}>
          <View style={styles.feedbackContainer}>
            <MaterialIcons
              ...
              color={colors.primary}
            />
            <AppText style={styles.feedbackText}>{errorMessage}</AppText>
            <Pressable
              accessibilityRole="button"
    ...
    return (
      <AnimatedEntry delay={390} triggerKey={animationKey}>
        <View style={styles.emptyContainer}>
          <AnimatedPop delay={470} triggerKey={animationKey}>
            <IconFavoritos width={96} height={96} />
          </AnimatedPop>
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      ...
      <FlatList
        data={isLoading || errorMessage ? [] : filteredFavorites}
